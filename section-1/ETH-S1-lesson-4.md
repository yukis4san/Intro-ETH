...
世界中に、ブロックチェーン上にデータを保存する作業を行う「マイナー」や「バリデーター」と呼ばれる人々が存在します。この作業に対して、わたしたちは代金を支払います。

その代金は、通称ガス代と呼ばれ、イーサリアムのブロックチェーン上にデータを書き込む場合、わたしたちはいくらかの`$ETH`を彼らに支払います。

スマートコントラクトは、世界中の誰でもアクセスできるさまざまな機能を備えたサーバー上に存在するコードのようなものです。
...
自動的に `0` に初期化される `totalWaves` 変数が追加されました。この変数は「状態変数」と呼ばれ、`WavePortal` コントラクトのストレージに永続的に保存されます。[unit256](https://www.iuec.co.jp/blockchain/uint256.html)は、非常に大きな数を扱うことができる「独自定義のクラス」を意味します。
...
これは、Solidityのアクセス修飾子の一つです。

Solidity含め様々な言語において、各関数のアクセス権限について指定する必要があります。その指定を行うのが、アクセス修飾子です。

Solidityには、4つのアクセス修飾子が存在します。

1. **`public`**: `public`で定義された関数や変数は、それらが定義されているコントラクト、そのコントラクトが継承された別のコントラクト、それらコントラクトの外部と、**基本的にどこからでも**の呼び出すことができます。*Solidityでは、アクセス修飾子がついてない**関数**を、自動的に`public`として扱われます。*
...
*Solidityでは、アクセス修飾子がついてない**変数**を、自動的に`internal`として扱われます。*
...
`msg.sender`に入る値は、ずばり、**関数を呼び出した人（＝あなたに「👋（wave）」を送った人）のウォレットアドレス**です。これは、ユーザー認証のようなものです。スマートコントラクトに含まれる関数を呼び出すには、ユーザーは有効なウォレットを接続する必要があります。`msg.sender`では、誰が関数を呼び出したかを正確に把握し、ユーザー認証を行っています。
...
Solidity開発では関数修飾子を意識しておかないとデータを記録する際のコスト（=ガス代）が跳ね上がってしまうので注意が必要です。ここでポイントとなるのは、**ブロックチェーンに値を書き込むにはガス代を払う必要があること、そしてブロックチェーンから値を参照するだけなら、ガス代を払う必要がないことです。**
...
以下に、Solidityの`pure`と`view`についてまとめています。
...
`wave()`関数には関数修飾子がついていないので、同一のユーザーが送った「👋（wave）」の回数が`totalWaves += 1`によってカウントされ、ブロックチェーン上にデータが書き込まれます。また、この関数が呼び出されると、`console.log("%s has waved!", msg.sender)`によって、あなたに「👋（wave）」を送ったユーザーのアドレスがターミナル上に表示されます。
...
`hre.ethers.getSigners()`はHardhatが提供するランダムなアドレスを生成する関数です。ここでは、コントラクト所有者（=あなた）のウォレットアドレスと、あなたに「👋（wave）」を送るユーザーのウォレットアドレスをHardhatがそれぞれ生成し、`owner`と`randomPerson`という変数に格納しています。`randomPerson`は、このレッスンの後半に登場するので、一旦このままレッスンを進めてください
。
...
ここには、あなたのスマートコントラクトのデプロイ先のアドレスを確認しています。
...
更にここでは、`WavePortal`コントラクトをデプロイした人（=あなた）のアドレスを確認しています。
...
`scripts`ディレクトリに移動し、`run.js`を実行してみましょう。
...
ここでは、他のユーザーがあなたに「👋（wave）」を送った場合のシミューションを行います。下記を`run.js`に反映させて、ターミナルにどのような結果がでるか確認してみましょう。
...
このレッスンの序盤で、`run.js`の中で`randomPerson`のアドレスを取得したのを覚えていますか？`randomPerson`には、Hardhatが取得したランダムなアドレスが格納されています。`randomPerson`は、このシミューションのために存在していたのです。
...
それでは、`run.js`を更新して、コードを実行してみましょう。
...
コードを少しカスタマイズしてください!! 多分あなたは何か他のものを保存したいですか？いじってほしい。送信者のアドレスを配列に格納したい場合がありますか？住所とウェーブカウントのマップを保存して、誰が最も手を振っているのかを追跡したい場合がありますか？変数名と関数名を変更して面白いと思うものに変更したとしても、それは大きな問題です。私を真っ直ぐにコピーしないようにしてください!最終的なWebサイトと必要な機能の種類を考えてください。**必要な**機能を構築します 。

ここですべて完了したら、ターミナル出力のスクリーンショットを#progressに投稿してください。-->
